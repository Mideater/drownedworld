...
import { queueUndoData, startUndoWorkflow, updateUndoChatCards, updateUndoConcentration, _undoMostRecentWorkflow } from "./undo.js";
...
export let processUndoDamageCard = (message, html, data) => {
	...
	message.flags.midiqol.undoDamage.forEach(({ actorUuid, oldTempHP, oldHP, totalDamage, newHP, newTempHP, oldVitality, newVitality, damageItem }) => {
		...
		select.change((ev) => {
			...
			if (multiplier === "calc")
				button.click(async (ev) => await doMidiClick(ev, actorUuid, newTempHP, newHP, newVitality, 1, data));
