...
async function doRollSkill(wrapped, ...args) {
	...
	if (chatMessage)
		displayDSNForRoll(result, "skill");
	if (!options.simulate) {
...
async function checkWounded(actor, update, options, user) {
	...
	if (hpUpdate === undefined && (!vitalityReosurce || vitalityUpdate === undefined))
		return;
