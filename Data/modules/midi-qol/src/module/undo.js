...
export async function updateUndoConcentrationData(workflow, concentrationData) {
...
export async function recordUndoData(undoData) {
...
export async function _undoEntry(data) {
	...
	if (effectsToRemove?.length) {
		...
		try {
			if (effectsToRemove.length)
				await tokendoc.actor.deleteEmbeddedDocuments("ActiveEffect", effectsToRemove);
		}
		catch (err) { }
	}
	let effectsToAdd;
	...
	if (!tokendoc?.actorLink && tokenChanges && !isEmpty(tokenChanges)) {
		...
		if (effectsToAdd?.length)
			await tokendoc.actor.createEmbeddedDocuments("ActiveEffect", effectsToAdd);
