...
*/
export function findNearby(disposition, token /*Token | uuuidString */, distance, options = { maxSize: undefined, includeIncapacitated: false, canSee: false }) {
	if (!token)
		return [];
		...
		const callback = async (dialog, button) => {
			...
			switch (button.value) {
				case "reroll":
					...
					if (showDiceSoNice)
						await displayDSNForRoll(reRoll, rollId);
					newRoll = reRoll;
					break;
				case "reroll-kh":
					reRoll = await this[rollId].reroll({ async: true });
					if (showDiceSoNice)
						await displayDSNForRoll(reRoll, rollId === "attackRoll" ? "attackRollD20" : rollId);
					newRoll = reRoll;
				...
				case "reroll-max":
					...
					if (showDiceSoNice)
						await displayDSNForRoll(newRoll, rollId === "attackRoll" ? "attackRollD20" : rollId);
					break;
				case "reroll-min":
				...
				default:
					if (typeof button.value === "string" && button.value.startsWith("replace ")) {
						...
						if (showDiceSoNice)
							await displayDSNForRoll(newRoll, rollId);
					}
						...
						if (showDiceSoNice)
							await displayDSNForRoll(tempRoll, rollId);
...
export async function setReactionUsed(actor) {
	if (configSettings.enforceReactions !== "all" && configSettings.enforceReactions !== actor.type)
		return;
	let effect;
...
export async function setBonusActionUsed(actor) {
	if (configSettings.enforceBonusActions !== "all" && configSettings.enforceBonusActions !== actor.type)
		return;
	let effect;
...
export async function displayDSNForRoll(roll, rollType) {
	if (!roll)
		return;
	/*
