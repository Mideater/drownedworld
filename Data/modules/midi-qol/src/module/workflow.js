		...
		switch (newState) {
			...
			case WORKFLOWSTATES.ALLROLLSCOMPLETE:
				...
				*/
				...
				if (configSettings.allowUseMacro)
					await this.triggerTargetMacros(["preApplyTargetDamage"], this.hitTargets);
			...
			case WORKFLOWSTATES.ROLLFINISHED:
				...
				if (!this.aborted) {
					...
					if (hasConcentration && checkConcentration) {
						...
						if (configSettings.undoWorkflow)
							await updateUndoConcentrationData(this, concentrationData);
					}
	...
	async triggerTargetMacros(triggerList, targets = this.targets) {
		for (let target of targets) {
			...
			if (wasAttacked && triggerList.includes("isAttacked")) {
				//@ts-ignore
				await this.callMacros(this.item, actorOnUseMacros?.getMacros("isAttacked"), "OnUse", "isAttacked", { actor: target.actor, token: target });
			}
			if (wasDamaged && triggerList.includes("isDamaged")) {
				await this.callMacros(this.item, actorOnUseMacros?.getMacros("isDamaged"), "OnUse", "isDamaged", { actor: target.actor, token: target });
			}
			if (wasHit && triggerList.includes("isHit")) {
				await this.callMacros(this.item, actorOnUseMacros?.getMacros("isHit"), "OnUse", "isHit", { actor: target.actor, token: target });
			}
			if (wasHit && triggerList.includes("preApplyTargetDamage")) {
				await this.callMacros(this.item, actorOnUseMacros?.getMacros("preApplyTargetDamage"), "OnUse", "preApplyTargetDamage", { actor: target.actor, token: target });
			}
			if (this.saveItem?.hasSave && triggerList.includes("preTargetSave") && this.saves.has(target)) {
				await this.callMacros(this.item, actorOnUseMacros?.getMacros("preTargetSave"), "OnUse", "preTargetSave", { actor: target.actor, token: target });
			}
			...
			if (this.saveItem?.hasSave && triggerList.includes("isSaveSuccess") && this.saves.has(target)) {
				await this.callMacros(this.item, actorOnUseMacros?.getMacros("isSaveSuccess"), "OnUse", "isSaveSuccess", { actor: target.actor, token: target });
			}
			if (this.saveItem?.hasSave && triggerList.includes("isSaveFailure") && !this.saves.has(target)) {
				await this.callMacros(this.item, actorOnUseMacros?.getMacros("isSaveFailure"), "OnUse", "isSaveFailure", { actor: target.actor, token: target });
			}
			if (this.saveItem?.hasSave && triggerList.includes("isSave")) {
				await this.callMacros(this.item, actorOnUseMacros?.getMacros("isSave"), "OnUse", "isSave", { actor: target.actor, token: target });
			}
		}
	}
	...
	*/
	async checkSaves(whisper = false, simulate = false) {
		...
		try {
			...
			if (configSettings.allowUseMacro)
				await this.triggerTargetMacros(["preTargetSave"], allHitTargets);
		...
		for (let target of allHitTargets) {
			...
			if (configSettings.allowUseMacro)
				await this.triggerTargetMacros(["isSave", "isSaveSuccess", "isSaveFailure"], this.hitTargets);
			if (!results[i])
